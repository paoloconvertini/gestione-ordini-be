...
public class AmmortamentoCespiteService {
    ...
    public void calcola(FiltroCespite filtroCespite) {
        try {
            ...
            List<RegistroCespiteDto> cespitiAttivi = Cespite.find(query, params)
                    .project(RegistroCespiteDto.class)
                    .list();

            if (cespitiAttivi.isEmpty()) {
                Log.debug("Nessun cespite attivo trovato");
                return;
            }

            // =========================
            // 2. PRELOAD AMMORTAMENTI (NO N+1)
            // =========================
            List<String> ids = cespitiAttivi.stream()
                    .map(RegistroCespiteDto::getId)
                    .toList();

            List<AmmortamentoCespite> allAmm = AmmortamentoCespite.find(
                    "idAmmortamento in :ids AND anno < :a",
                    Parameters.with("ids", ids).and("a", anno)
            ).list();

            Map<String, List<AmmortamentoCespite>> ammMap =
                    allAmm.stream().collect(Collectors.groupingBy(AmmortamentoCespite::getIdAmmortamento));

            // =========================
            // 3. DELETE OTTIMIZZATA
            // =========================
            AmmortamentoCespite.delete("anno >= :a AND idAmmortamento in :ids",
                    Parameters.with("a", anno).and("ids", ids));

            // =========================
            // 4. CALCOLO
            // =========================
            List<AmmortamentoCespite> daSalvare = new ArrayList<>();

            Log.debug("Calcola inizio ciclo cespiti Attivi");

            for (RegistroCespiteDto cespite : cespitiAttivi) {

                List<AmmortamentoCespite> ammortamenti =
                        ammMap.getOrDefault(cespite.getId(), Collections.emptyList());

                Optional<AmmortamentoCespite> ammPrecAnno =
                        ammortamenti.stream()
                                .filter(am -> am.getAnno() == anno - 1)
                                .findFirst();

                boolean condizioneCalcolo =
                        (ammPrecAnno.isEmpty() && ammortamenti.isEmpty()) ||
                                (ammPrecAnno.isPresent() &&
                                        (cespite.getImporto() +
                                                (cespite.getImportoRivalutazione() != null ? cespite.getImportoRivalutazione() : 0))
                                                != (ammPrecAnno.get().getFondo() + ammPrecAnno.get().getFondoRivalutazione())
                                );

                if (condizioneCalcolo) {

                    if (cespite.getImportoRivalutazione() != null && cespite.getImportoRivalutazione() != 0) {
                        daSalvare.addAll(
                                calcoloSingoloCespiteRivalutato(cespite, dataCorrente, ammPrecAnno)
                        );
                    } else {
                        daSalvare.addAll(
                                calcoloSingoloCespite(cespite, dataCorrente, ammPrecAnno)
                        );
                    }
                }
            }

            Log.debug("FINE --- ciclo cespiti Attivi");
            ...
            for (int i = 0; i < daSalvare.size(); i++) {
                AmmortamentoCespite.persist(daSalvare.get(i));

                if (i % batchSize == 0) {
                    AmmortamentoCespite.getEntityManager().flush();
                    AmmortamentoCespite.getEntityManager().clear();
                }
            }
    ...
    public List<AmmortamentoCespite> ricalcoloCespite(RegistroCespiteDto cespite, LocalDate dataCorrente) {
        long inizio = System.currentTimeMillis();
    ...
    public List<AmmortamentoCespite> calcoloSingoloCespite(RegistroCespiteDto cespite, LocalDate dataCorrente, Optional<AmmortamentoCespite> optionalAmmortamentoCespite) {
        long inizio = System.currentTimeMillis();
        List<AmmortamentoCespite> ammortamentoCespiteList = new ArrayList<>();
        Double percAmm = cespite.getPercAmmortamento();
        boolean eliminato = cespite.getDataVendita() != null && StringUtils.isBlank(cespite.getIntestatarioVendita());
        ...
        if (percAmm != null && percAmm != 0) {
            ...
            if (optionalAmmortamentoCespite.isPresent()) {
                ...
                double perc;
                if (fondo >= cespite.getImporto()) {
                    quotaDaSalvare = ammortamentoCespite.getResiduo();
                    fondo = cespite.getImporto();
                    residuo = 0;
                } else {
                    residuo = ammortamentoCespite.getResiduo() - quotaDaSalvare;
                }
                if (cespite.getImporto() == 0) {
                    perc = 0;
                } else {
    ...
    public List<AmmortamentoCespite> ricalcoloRivalutato(RegistroCespiteDto cespite, LocalDate dataCorrente) {
        Double percAmm = cespite.getPercAmmortamento();
        List<AmmortamentoCespite> ammortamentoCespiteList = new ArrayList<>();
        boolean eliminato = cespite.getDataVendita() != null && StringUtils.isBlank(cespite.getIntestatarioVendita());
        boolean venduto = cespite.getDataVendita() != null && StringUtils.isNotBlank(cespite.getIntestatarioVendita());
        ...
        if (percAmm != null && percAmm != 0) {
            ...
            while (residuo > 0 && dataAmmortamento.getYear() <= dataCorrente.getYear()) {
                ...
                quotaRivDaSalvare = quotaRiv * dataAmmortamento.getDayOfYear() / (dataAmmortamento.isLeapYear() ? 366 : 365);
    ...
    public List<AmmortamentoCespite> calcoloSingoloCespiteRivalutato(RegistroCespiteDto cespite, LocalDate dataCorrente, Optional<AmmortamentoCespite> optionalAmmortamentoCespite) {
        Double percAmm = cespite.getPercAmmortamento();
        List<AmmortamentoCespite> ammortamentoCespiteList = new ArrayList<>();
        boolean eliminato = cespite.getDataVendita() != null && StringUtils.isBlank(cespite.getIntestatarioVendita());
        boolean venduto = cespite.getDataVendita() != null && StringUtils.isNotBlank(cespite.getIntestatarioVendita());
        ...
        if (percAmm != null && percAmm != 0) {
            ...
            if (optionalAmmortamentoCespite.isPresent()) {
                ...
                residuoDaSalvare = cespite.getImportoRivalutazione() - fondoRiv;
            }
            if (cespite.getImporto() == 0) {
                perc = 0;
            } else {
    ...
    private List<AmmortamentoCespite> buildEliminatoVenduto(RegistroCespiteDto cespite, List<AmmortamentoCespite> ammortamentoCespiteList, boolean eliminato, boolean venduto, double residuo) {
        ...
        if (eliminato || venduto || (cespite.getResiduo() != null && cespite.getResiduo() == 0)) {
            Cespite.update("attivo = 'F' WHERE id=:id", Parameters.with("id", cespite.getId()));
        }
        return ammortamentoCespiteList;
    }

    private void calcolaSuperAmm(RegistroCespiteDto cespite, double perc, AmmortamentoCespite a) {
        calcolaSuperAmm(perc, a, cespite.getSuperAmm(), cespite.getImporto());
    }
    ...
    public RegistroCespitiDto getRegistroCespiti(FiltroCespite filtroCespite) {
        ...
        Map<Long, TipoSuperAmm> superAmmLookup = new HashMap<>();
        for (TipoSuperAmm t : TipoSuperAmm.<TipoSuperAmm>listAll()) {
            superAmmLookup.put(t.id, t);
        }
        ...
        try {
            String query = "SELECT c.id, c.tipoCespite, c.progressivo1, c.progressivo2, c.cespite, c.dataAcq, c.numDocAcq, c.fornitore, " +
                    "c.importo, c.importoRivalutazione, c.attivo, c.dataVendita, c.numDocVendita, c.intestatarioVendita, c.importoVendita," +
                    " c.superAmm, c.protocollo, c.giornale, c.anno, c.dataInizioCalcoloAmm, c.flPrimoAnno, c.fondoRivalutazione, a.idAmmortamento, a.dataAmm, " +
            ...
            Map<String, Map<Integer, Map<Integer, List<RegistroCespiteDto>>>> struttura = new HashMap<>();
            for (RegistroCespiteDto dto : cespiteDtos) {
                String tipo = dto.getTipoCespite();
                Integer p1 = dto.getProgressivo1();
                Integer p2 = dto.getProgressivo2();
                struttura
            ...
            for (String tipoCespite : struttura.keySet()) {
                Map<Integer, Map<Integer, List<RegistroCespiteDto>>> progr1Map = struttura.get(tipoCespite);
                List<RegistroCespiteDto> dtoList = new ArrayList<>();
                for (Map<Integer, List<RegistroCespiteDto>> m1 : progr1Map.values()) {
                    for (List<RegistroCespiteDto> m2 : m1.values()) {
                        dtoList.addAll(m2);
                    }
                }
                ...
                for (Integer progressivo : progr1Map.keySet()) {
                    Map<Integer, List<RegistroCespiteDto>> progr2Map = progr1Map.get(progressivo);
                    ...
                    for (Integer progr2 : progr2Map.keySet()) {
                        ...
                        if (dbDto1.getSuperAmm() != null && dbDto1.getSuperAmm() != 0L) {
                            TipoSuperAmm tipoSuperAmm = superAmmLookup.get(dbDto1.getSuperAmm());
                            if (tipoSuperAmm != null) {
                                v.setSuperAmmDesc(tipoSuperAmm.getDescrizione());
                            }
                        }
                        List<AmmortamentoCespite> list = new ArrayList<>();
                        for (RegistroCespiteDto d : progr2List) {
                            AmmortamentoCespite a = mapper.buildAmmortamento(d);
                            if (a != null) {
                                list.add(a);
                            }
                        }
                        List<AmmortamentoCespite> collect = new ArrayList<>();
                        for (AmmortamentoCespite a : list) {
                            if (a.getDataAmm() != null &&
                                    !StringUtils.startsWith(a.getDescrizione(), "VENDITA")) {
                                collect.add(a);
                            }
                        }
                        collect.sort(Comparator.comparing(AmmortamentoCespite::getDataAmm));
                        if (!dbDto1.getAttivo()) {
                ...
                Map<String, List<CespiteDto>> superAmmMap = new HashMap<>();
                for (CespiteDto c : cespiteDtoList) {
                    if (StringUtils.isNotBlank(c.getSuperAmmDesc())) {
                        superAmmMap
                                .computeIfAbsent(c.getSuperAmmDesc(), k -> new ArrayList<>())
                                .add(c);
                    }
                }
                for (String superAmmDesc : superAmmMap.keySet()) {
                    ...
                    List<AmmortamentoCespite> list = new ArrayList<>();
                    for (CespiteDto c : viewDtoList) {
                        list.addAll(c.getAmmortamentoCespiteList());
                    }
                    ...
                    TipoSuperAmm t = null;
                    for (TipoSuperAmm tipo : superAmmLookup.values()) {
                        if (superAmmDesc.equals(tipo.getDescrizione())) {
                            t = tipo;
                            break;
                        }
                ...
                for (CespiteDto c : cespiteDtoList) {
                    if (c.getImportoVendita() != null && c.getDataVend().getYear() == anno) {
                        AmmortamentoCespite max = null;
                        for (AmmortamentoCespite a : c.getAmmortamentoCespiteList()) {
                            if (StringUtils.startsWith(a.getDescrizione(), "Ammortamento")) {
                                if (max == null || a.getAnno() > max.getAnno()) {
                                    max = a;
                                }
                            }
                        }
                        if (max != null) {
                            ammortamentoCespitesVend.add(max);
                        }
                    }
                }
                ...
                double plus = 0;
                for (AmmortamentoCespite a : ammortamentoCespiteList2) {
                    if (a.getAnno() == anno && StringUtils.startsWith(a.getDescrizione(), "Plus")) {
                        plus += a.getQuota();
                    }
                }
                if (plus != 0) {
            ...
            if (result.stream().anyMatch(c -> c.getSomma().getSuperAmm1() != null && c.getSomma().getSuperAmm1().getTotale() != null)) {
                TipoSuperAmm tipo1 = superAmmLookup.get(1L);
            ...
            if (result.stream().anyMatch(c -> c.getSomma().getSuperAmm2() != null && c.getSomma().getSuperAmm2().getTotale() != null)) {
                TipoSuperAmm tipo2 = superAmmLookup.get(2L);
            ...
            if (result.stream().anyMatch(c -> c.getSomma().getSuperAmm3() != null && c.getSomma().getSuperAmm3().getTotale() != null)) {
                TipoSuperAmm tipo3 = superAmmLookup.get(3L);
                sommaDto.setSuperAmm3(new SuperAmmDto(tipo3.getDescrizione(), result
                        .stream()
        ...
        if (optionalCespite.isPresent()) {
            ...
            if (update > 0) {
                ...
                if (cespiteDBDto.getImportoRivalutazione() != null) {
                    ammortamentoCespites = ricalcoloRivalutato(cespiteDBDto, LocalDate.now());
                } else {
                    ammortamentoCespites = ricalcoloCespite(cespiteDBDto, LocalDate.now());
                }
    ...
    private void calcolaSuperAmm(Double percAmm, AmmortamentoCespite a, Long superAmm, Double importo) {
        if (superAmm != null && superAmm != 0L) {
            TipoSuperAmm tipoSuperAmm = TipoSuperAmm.findById(superAmm);
            double importSuperAmm = (tipoSuperAmm.getPerc() * importo) / 100;
            double quotaSuperAmm = (percAmm * importSuperAmm) / 100;
            if (a.getResiduo() < quotaSuperAmm) {
                quotaSuperAmm = a.getResiduo();
            }
            a.setSuperPercentuale(percAmm);
    ...
    @Transactional
    public void aggiornaAmmortamenti(AmmortamentoCespite a, LocalDate data) {
        AmmortamentoCespite.delete("idAmmortamento = :id and anno >=:a", Parameters.with("id", a.getIdAmmortamento()).and("a", a.getAnno()));
        Cespite c = Cespite.find("id = :id", Parameters.with("id", a.getIdAmmortamento())).firstResult();
        CategoriaCespite tipo = CategoriaCespite.find("tipoCespite = :t", Parameters.with("t", c.getTipoCespite())).firstResult();
        RegistroCespiteDto d = mapper.fromCespiteToRegistroCespiteDto(c, tipo);
        List<AmmortamentoCespite> ammortamentoCespiteList = new ArrayList<>();
        if (c.getImportoRivalutazione() != null && c.getImportoRivalutazione() != 0) {
            ammortamentoCespiteList.addAll(calcolaAmmortamentiRivalutati(d, a, data));
        } else {
            ammortamentoCespiteList.addAll(calcolaAmmortamenti(d, a, data));
        }
    ...
    private List<AmmortamentoCespite> calcolaAmmortamentiRivalutati(RegistroCespiteDto cespite, AmmortamentoCespite a, LocalDate dataCorrente) {
        Double percAmm = cespite.getPercAmmortamento();
        List<AmmortamentoCespite> ammortamentoCespiteList = new ArrayList<>();
        ...
        while (residuo > 0 && dataAmmortamento.getYear() <= dataCorrente.getYear()) {
            ...
            if (counter == 1) {
                ...
                quotaRivDaSalvare = a.getQuotaRivalutazione();
            } else {
                quotaDaSalvare = quota * dataAmmortamento.getDayOfYear() / (dataAmmortamento.isLeapYear() ? 366 : 365);
                quotaRivDaSalvare = quotaRiv * dataAmmortamento.getDayOfYear() / (dataAmmortamento.isLeapYear() ? 366 : 365);
            }
            ...
            AmmortamentoCespite amm = mapper.buildAmmortamento(cespite.getId(), perc, quotaDaSalvare, quotaRivDaSalvare, fondo, fondoRiv, residuoDaSalvare, dataAmmortamento);
    ...
    private List<AmmortamentoCespite> calcolaAmmortamenti(RegistroCespiteDto cespite, AmmortamentoCespite a, LocalDate dataCorrente) {
        Double percAmm = cespite.getPercAmmortamento();
        List<AmmortamentoCespite> ammortamentoCespiteList = new ArrayList<>();
        Optional<AmmortamentoCespite> optAmmPrec = AmmortamentoCespite.find("idAmmortamento = :id AND anno =:a",
                Parameters.with("id", a.getIdAmmortamento()).and("a", a.getAnno() - 1)).firstResultOptional();
